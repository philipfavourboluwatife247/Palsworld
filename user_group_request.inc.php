	...
	<a href="<?=ROOT?>profile/<?php echo $FRIEND_ROW['userid']; ?>">
		...
		<?php 
			...
			if($invited && isset($INVITER_ROW)){
				...
				echo "You were invited by " . $INVITER_ROW['username'] "<br><br>";
			}
		?>
		<a href="<?=ROOT?>group_request_accept/<?=$group_data['userid']?>/<?=$FRIEND_ROW['userid']?>/decline">
			<input id="post-button" type="button" value="Decline" style="float:right;margin-right:10px;background-color: #916e1b;width:auto;">
		</a>
		<a href="<?=ROOT?>group_request_accept/<?=$group_data['userid']?>/<?=$FRIEND_ROW['userid']?>/accept">
			<input id="post-button" type="button" value="Accept" style="float:left;margin-right:10px;background-color: #1b9186;width:auto;">
		</a>
