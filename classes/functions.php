...
function i_own_content($row){
	...
	$myid = $_SESSION['palsworld_userid'];
	
	//profiles
	if(isset($row['username']) && $row['type'] == "profile" && $myid == $row['userid']){

		return true;
	}

	//groups
	if(isset($row['username']) && $row['type'] == "group" && $myid == $row['owner']){
...
function tag($postid,$new_post_text = "")
{
	...
	$mypost = $Database->read_from_db($sql);;
			...
			if(is_array($tagged_user)){
				...
				if($new_post_text != ""){
					...
					if(!in_array($tagged_user['tag_name'], $old_tags)){
						add_notification($_SESSION['palsworld_userid'],"tag",$mypost,$tagged_user['userid']);
					}
				}else{
...
function notification_seen($id)
{
	...
	$userid = $_SESSION['palsworld_userid'];
	$Database = new Automated_DB();
	...
	if(is_array($words) && count($words)>0)
	{
		...
		foreach ($words as $word) {
			...
			if(preg_match("/@[a-zA-Z_0-9\Q,.\E]+/", $word)){
				...
				$user_row = $Database->read_from_db($query);
...
if(isset($_SESSION['palsworld_userid'])){
	set_online($_SESSION['palsworld_userid']);
}
