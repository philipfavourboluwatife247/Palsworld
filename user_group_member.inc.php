...
<div id="friends" style="display: inline-block;vertical-align: top; width: 200px;background-color: #eee;">
	...
	<a href="<?=ROOT?>profile/<?php echo $FRIEND_ROW['userid']; ?>">
		...
		<?php if(group_access($_SESSION['palsworld_userid'],$group_data,'admin')):?>
			<br style="clear: both;">
			<a href="<?=ROOT?>group/<?=$group_data['userid']?>/members?remove=<?=$FRIEND_ROW['userid']?>">
				<input id="post-button" type="button" value="Remove" style="font-size:11px;margin-right:10px;background-color: #916e1b;width:auto;">
			</a>
			<a href="<?=ROOT?>group/<?=$group_data['userid']?>/members?edit_access=<?=$FRIEND_ROW['userid']?>">
				<input id="post-button" type="button" value="Edit Access" style="font-size:11px;margin-right:10px;background-color: #1b9186;width:auto;">
			</a>
	 	<?php endif;?>
 	</a>
</div>